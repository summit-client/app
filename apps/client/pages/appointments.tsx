...
export const getServerSideProps: GetServerSideProps<PageProps> = async ({
  ...
  // The family comes first: both queries below filter on the children this
  // guardian may see, and the change-request list used to be keyed on the
  // viewed child alone even though the sessions on this page are every
  // child's. That cost no round trip to fix - it runs beside the sessions
  // query instead of beside this one, so the page is still two hops deep.
  ...
  const [sessionsRes, changeRequestsRes] = await Promise.all([
    supabase
    .from("sessions")
    .select(`
      id,
    ...
    // Every reschedule/cancel request for every child on this page, newest
    // first - pages/api/sessions/request-change.ts is the only writer, and it
    // stamps the row with the SESSION's client_id. This was
    // .eq("client_id", viewed.clientId), so a request filed against a
    // sibling's session never reached latestRequestBySession: the card showed
    // "Request reschedule" again instead of the awaiting-staff pill, telling
    // the family nothing was pending and inviting a duplicate.
    //
