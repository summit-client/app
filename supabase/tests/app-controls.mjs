...
const PUBLIC_ROUTES = {
  ...
  // Reviewed and declared 2026-09-19. This route did not exist when this list
  // was written; the scheduler grew its own feed after apps/client's.
  "apps/scheduler/pages/api/calendar/feed/[token].ics.ts":
    "The scheduler's own token-gated feed, same trust model as apps/client's above and documented at length in the route itself: token -> calendar_feed_tokens row -> clinic_id read directly off that row, never off a query parameter. A leaked personal token exposes one person's own sessions; a front-desk token exposes scrubbed rows only. Nothing beyond the token is accepted as input.",
};
...
const SERVICE_ROLE_ALLOWED = {
  ...
  // Reviewed and declared 2026-09-19, same reason as apps/client's above: the
  // scheduler grew its own feed after this list was written, and a route whose
  // caller is unauthenticated by construction has no session to act as.
  "apps/scheduler/lib/calendar-feed-tokens.ts":
    "Resolves a feed token to its row so the route can scope every later query by the clinic_id on that row. The caller is a calendar app carrying no session, so there is nothing else to act as.",
};
